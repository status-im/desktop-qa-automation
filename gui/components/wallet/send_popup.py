...
class SendPopup(BasePopup):
    ...
    def __init__(self):
        ...
        self._token_list_item = QObject('o_TokenBalancePerChainDelegate_template')
        ...
        self._arbitrum_amount_label = TextLabel('statusListItemSubTitle_StatusTextWithLoadingState')
    ...
    def _select_token(self, token: str):
        item = wait_for_template(self._token_list_item.real_name, token, 'title')
    ...
    @allure.step('Send {2} token {3} to {1}')
    def send(self, address: str, amount: int, token: str):
        self._open_tab('Assets')
        self._search_field.type_text(token)
    ...
    def sending_amount(self) -> str:
        return self._arbitrum_amount_label.text
