...
@pytest.mark.parametrize('tab, receiver_account_address, amount, asset', [
    pytest.param('Assets', constants.user.user_account_one.status_address, 0, 'Ether')
])
@pytest.mark.xfail(reason="https://github.com/status-im/status-desktop/issues/12987")
def test_wallet_send_0_eth(keys_screen, main_window, user_account, tab, receiver_account_address, amount, asset):
    ...
    with step('Enter asset, amount and address on Assets tab, click send and verify Mainnet network is shown'):
        send_popup.send(tab, receiver_account_address, amount, asset)
...
@pytest.mark.parametrize('tab, receiver_account_address, amount, collectible', [
    pytest.param('Collectibles', constants.user.user_account_one.status_address, 1, 'Goerli_NFTS #27853')
])
...
def test_wallet_send_nft(keys_screen, main_window, user_account, tab, receiver_account_address, amount, collectible):
    ...
    with step('Enter asset, amount and address on Collectibles tab, click send and verify Mainnet network is shown'):
        send_popup.send(tab, receiver_account_address, amount, collectible)
        assert driver.waitFor(lambda: send_popup.is_mainnet_network_identified, configs.timeouts.UI_LOAD_TIMEOUT_SEC)
...
@allure.testcase('https://ethstatus.testrail.net/index.php?/cases/view/',
                 '')
@pytest.mark.case()
@pytest.mark.parametrize('user_account', [constants.user.user_with_funds])
@pytest.mark.parametrize('tab, receiver_account_address, amount, asset', [
    pytest.param('Assets', constants.user.user_account_one.status_address, 1, 'Status Test Token')
])
@pytest.mark.skip(reason="https://github.com/status-im/status-desktop/issues/12987")
def test_wallet_send_snt(keys_screen, main_window, user_account, tab, receiver_account_address, amount, asset):
    with step('Open import seed phrase view and enter seed phrase'):
        input_view = keys_screen.open_import_seed_phrase_view().open_seed_phrase_input_view()
        input_view.input_seed_phrase(user_account.seed_phrase, True)
        profile_view = input_view.import_seed_phrase()
